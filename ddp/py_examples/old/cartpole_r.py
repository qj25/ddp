...
class CartPoleREnv(gymnasium.Env[np.ndarray, Union[int, np.ndarray]]):
    """
    ### Description
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    # CITE1 why not use theta instead of its trigs
    def step(self, action, constrain=True):
        # err_msg = f"{action!r} ({type(action)}) invalid"
        # assert self.action_space.contains(action), err_msg
        assert self.state is not None, "Call reset before using step method."
        x_, x_dot, theta, theta_dot = self.state
        F = np.tanh(action) if constrain else action
    ...
    def reset(
        ...
        *,
        ...
        # self.state = np.array([0.0, 0.0, np.sin(np.pi), np.cos(np.pi), 0.0])
